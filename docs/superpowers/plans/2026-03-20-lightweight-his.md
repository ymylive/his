...
- 说明：第一轮已完成控制台菜单控制器、角色路由、非法输入回退和统一反馈。第二轮已新增 `MenuApplication` 作为薄应用层，将 `main.c` 保持为菜单循环入口，并把一批已有 service 能稳定支撑的动作接到真实业务执行。
- 当前已接通动作：
- 挂号员：添加患者、修改患者信息、查询患者、挂号办理、挂号查询/取消
- 医生：待诊列表、查询患者历史、诊疗记录写入、检查记录新增与结果回写、药品库存查询
- 住院登记员/病区管理员：查看病房、查看床位、办理入院、办理出院、按患者查当前住院、按床位查当前入住患者
- 药房：添加药品、药品入库、发药、库存查询、库存不足提醒
- 患者：按患者编号查询挂号记录
- 当前仍保留为后续迭代的动作：
- 管理员综合维护入口
- 患者侧发药记录、药品使用方法
...
- [x] **Step 1: 写菜单路由与非法输入回退测试**
- [x] **Step 2: 为管理员、挂号员、医生、患者、住院登记员、病区管理员、药房人员提供独立菜单**
- [x] **Step 3: 每次操作后统一输出编号、状态和下一步提示**
- [x] **Step 4: 为患者菜单增加挂号查询、个人看诊/检查/住院/发药历史与药品使用方法入口，并为住院菜单增加病区床位查询、入院登记、出院办理入口**
- [x] **Step 5: 运行 `ctest --test-dir build --output-on-failure`**
- [ ] **Step 6: 提交**
