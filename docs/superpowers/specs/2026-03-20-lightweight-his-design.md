...
> **鲁棒型门诊 + 基础住院 HIS**
>
> 重点不在“大而全”，而在于对异常输入、边界情况、库存一致性、收费闭环、床位状态和日志可追踪性的处理足够稳健。
...
3. 收费员根据就诊单生成账单并收费。
4. 药房根据已缴费处方发药并自动扣减库存。
5. 符合住院条件的患者办理入院，系统分配床位并生成住院号。
6. 住院期间记录日费用、预交金与床位状态。
7. 结清费用后办理出院，释放床位。
8. 管理员查看日报、药品预警、床位占用率和操作日志。
...
- `Application 层`：业务编排，驱动挂号、收费、发药等用例
...
| `data/bills.txt` | 账单信息 | billId\|regId\|amount\|status\|paidAt |
| `data/wards.txt` | 病房/病区信息 | wardId\|wardName\|wardType\|deptId\|floor |
| `data/beds.txt` | 床位信息 | bedId\|wardId\|bedNo\|status\|currentPatientId\|pricePerDay |
| `data/admissions.txt` | 住院记录 | admissionId\|patientId\|doctorId\|bedId\|admitDate\|status |
| `data/inpatient_orders.txt` | 住院费用/医嘱记录 | orderId\|admissionId\|type\|amount\|note\|date |
| `data/inpatient_bills.txt` | 住院结算 | inpatientBillId\|admissionId\|deposit\|totalCost\|balance\|status |
...
- 未收费账单不能发药。
- 已收费处方发药成功后自动扣减库存。
- 若库存不足，发药失败并保留账单状态，提示人工处理。
- 退费必须先校验是否已发药，已发药账单退费需要走撤销发药流程。
...
sequenceDiagram
    ...
    AdmitSvc-->>Admin: 返回住院编号、床位编号、预交金提示
```
...
- `管理视角`：查看科室收入、医生接诊量、药品库存、床位占用率
- `患者视角`：查看个人挂号、就诊、检查、住院与费用记录
...
- 对住院号、床位号、预交金额做存在性与范围校验。

### 7.2 操作防呆

- 关键操作二次确认，例如退费、删除患者、清空数据。
- 入院、转床、出院结算等关键操作必须二次确认。
- 对不可逆动作给出原因说明。
...
| 类别 | 例子 |
| --- | --- |
| 正常用例 | 建档、挂号、接诊、收费、发药成功 |
| 住院流程 | 入院、分配床位、记录费用、追加预交金、出院结算 |
| 边界用例 | 年龄为 0、库存刚好为 1、账单金额为 0 |
| 异常用例 | 输入负数数量、未知患者编号、重复收费、重复入院 |
| 状态用例 | 未收费先发药、已发药后直接退费、未结清先出院、占用床位重复分配 |
