...
static Result WardRepository_validate(const Ward *ward) {
    if (ward == 0) {
        return Result_make_failure("ward missing");
    }
...
static Result WardRepository_format_line(
    ...
    if (line == 0 || line_capacity == 0) {
        return Result_make_failure("ward line buffer missing");
    }
...
static Result WardRepository_parse_line(const char *line, Ward *ward) {
    ...
    if (line == 0 || ward == 0) {
        return Result_make_failure("ward line missing");
    }
...
static Result WardRepository_ensure_header(const WardRepository *repository) {
    ...
    if (repository == 0) {
        return Result_make_failure("ward repository missing");
    }
    ...
    if (file == 0) {
        return Result_make_failure("failed to inspect ward repository");
    }
...
static Result WardRepository_collect_line(const char *line, void *context) {
    ...
    if (ward == 0) {
        return Result_make_failure("failed to allocate ward");
    }
...
static int WardRepository_validate_list(const LinkedList *wards) {
    ...
    while (outer != 0) {
        ...
        while (inner != 0) {
            ...
            if (strcmp(ward->ward_id, other->ward_id) == 0) {
                return 0;
            }
...
Result WardRepository_load_all(
    ...
    if (out_wards == 0) {
        return Result_make_failure("ward output list missing");
    }
...
Result WardRepository_save_all(
    ...
    if (file == 0) {
        return Result_make_failure("failed to rewrite ward repository");
    }
